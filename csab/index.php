...
$sql = "SELECT DISTINCT TRIM(SUBSTRING_INDEX(Institute, ',', 1)) AS College, State, Branch FROM csab";
...
<head>
  ...
  <title>Mock CSAB Choice Filler</title>
  <link rel="icon" href="assets/images/clogo.png" type="image/x-icon">
...
<header class="bg-blue-800 text-white p-4 shadow-lg flex justify-between items-center">
  <div class="flex items-center">
    <img src="assets/images/clogo.png" alt="Logo" class="h-8 mr-2">
    <h1 class="text-3xl font-bold">Mock CSAB Choice Filler</h1>
  </div>
  <div class="flex items-center space-x-4">
    ...
    <a href="https://www.linkedin.com/in/arkagrawal/" target="_blank" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline transition duration-150 ease-in-out transform hover:scale-105">Join</a>
  </div>
</header>

<div id="disclaimer" class="container mx-auto p-4 flex-grow-0">
  <div class="bg-yellow-200 p-4 rounded-lg shadow-lg">
    <p class="text-center text-lg font-bold">Disclaimer: This is a mock choice filling system for CSAB. The choices you make here will not affect your actual seat allotment. Use this tool for practice only. Please enter your real name and rank using mock names can lead to ban and will not save your data.</p>
  ...
  <div class="bg-white p-8 rounded-lg shadow-lg">
    ...
    <form id="userForm">
      <div class="mb-4">
        ...
        <p class="text-xs text-red-500">Use the same name and rank combination to view your choices next time.</p>
      </div>
      <div class="mb-4">
        <label for="rank" class="block text-gray-700 text-sm font-bold mb-2">Rank:</label>
        <input type="number" id="rank" name="rank" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
      </div>
...
<footer class="bg-blue-800 text-white p-4 mt-8 shadow-lg flex-shrink-0">
  <div class="container mx-auto flex flex-col md:flex-row justify-between items-center text-center md:text-left">
    <p class="mb-4 md:mb-0">&copy; 2024 CSAB Choice Filling Helper. All Rights Reserved.</p>
    <div class="flex space-x-4">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  leftSearchInput.addEventListener('keyup', function() {
    ...
    Array.from(rows).forEach(row => {
      ...
      const state = row.getElementsByTagName('td')[1].textContent.toLowerCase();
      const branch = row.getElementsByTagName('td')[2].textContent.toLowerCase();
      if (college.includes(filter) || state.includes(filter) || branch.includes(filter)) {
        row.style.display = '';
      } else {
  ...
  rightSearchInput.addEventListener('keyup', function() {
    ...
    Array.from(rows).forEach(row => {
      ...
      const state = row.getElementsByTagName('td')[2].textContent.toLowerCase();
      const branch = row.getElementsByTagName('td')[3].textContent.toLowerCase();
      if (college.includes(filter) || state.includes(filter) || branch.includes(filter)) {
        row.style.display = '';
      } else {
        row.style.display = 'none';
  ...
  document.addEventListener('click', function(event) {
    if (event.target.classList.contains('add-button')) {
      ...
      const state = row.getElementsByTagName('td')[1].textContent;
    ...
    if (event.target.classList.contains('remove-button')) {
      ...
      const state = row.getElementsByTagName('td')[2].textContent;
      const branch = row.getElementsByTagName('td')[3].textContent;
  ...
  function saveChoices() {
    ...
    Array.from(rows).forEach(row => {
      ...
      const state = row.getElementsByTagName('td')[2].textContent;
      const branch = row.getElementsByTagName('td')[3].textContent;
      choices.push({ college: college, state: state, branch: branch });
    });
  ...
  downloadPdfButton.addEventListener('click', function() {
    ...
    background.src = 'cmockby.png';
    background.onload = function() {
      doc.addImage(background, 'PNG', 0, 0, doc.internal.pageSize.getWidth(), doc.internal.pageSize.getHeight());

      doc.setFontSize(20);
      doc.text('Mock CSAB Choice Filler', 105, 20, { align: 'center' });
      doc.addImage('assets/images/clogo.png', 'PNG', 90, 25, 30, 30);
      ...
      doc.text("Join Me: https://www.linkedin.com/in/arkagrawal/", 10, 90);
      ...
      Array.from(rows).forEach(row => {
        ...
        const state = row.getElementsByTagName('td')[2].textContent;
        const branch = row.getElementsByTagName('td')[3].textContent;
        data.push([choiceNumber, college, state, branch]);
      });

      doc.autoTable({
        head: [['#', 'College', 'State', 'Branch']],
        body: data,
        startY: 110,
        theme: 'grid',
      ...
      doc.save('Mock_CSAB_Choice_Filler.pdf');
    };
  });
});
</script>
